...
class Zoombini {
  ...
  resizeImageZoom() {
    ...
    this.imageZoom.style.width =
      this.image.getBoundingClientRect().width * settings.magnification + "px";
  }
  ...
  drawMask() {
    ...
    let prop_x =
      (this.mousex / image.width) *
        imageZoom.width *
        (1 - 1 / settings.magnification) -
      image.x -
      window.scrollX;
    let prop_y =
      (this.mousey / image.height) *
        imageZoom.height *
        (1 - 1 / settings.magnification) -
      image.y -
      window.scrollY;
